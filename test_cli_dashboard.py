...
from engine.cli_dashboard import DashboardCLI


def build_cli_sample_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    rows = []
    ...
    for offset, (session, reason) in enumerate(
        [
            ("LONDON", "Spread too wide | mean=0.34"),
            ("OVERLAP", "Low tick ratio | 0.44"),
            ("NY", "Quality below threshold | 0.58"),
        ],
        start=len(completed_rows) + 1,
    ):
        rows.append(
            {
    ...
    completed_df = decision_df[decision_df["decision"] == "TRADE_TAKEN"].dropna(subset=["outcome"]).copy()
    return decision_df, completed_df


class DashboardCLITests(unittest.TestCase):
    def setUp(self):
        self.temp_dir = Path(tempfile.mkdtemp(prefix="phase4_cli_"))
        self.decision_df, self.completed_df = build_cli_sample_data()
        self.cli = DashboardCLI(
            days=14,
            refresh_seconds=5,
            output_root=str(self.temp_dir),
            backend="Agg",
            data_loader=lambda days: (self.decision_df.copy(), self.completed_df.copy()),
            sync_loader=lambda hours: {"synced": True, "message": f"mock sync {hours}h"},
        )

    def tearDown(self):
        shutil.rmtree(self.temp_dir, ignore_errors=True)

    def test_summary_text_contains_core_metrics(self):
        summary = self.cli.build_summary_text(
            self.decision_df,
            self.completed_df,
            {"synced": True, "message": "mock sync"},
        )

        self.assertIn("Phase 4 CLI Dashboard", summary)
        self.assertIn("Completed trades", summary)
        self.assertIn("Win rate", summary)
        self.assertIn("Total PnL", summary)

    def test_export_overview_snapshot_creates_png(self):
        result = self.cli.export_overview_snapshot(sync_first=False)
    ...
    def test_export_full_chart_suite_creates_bundle(self):
        result = self.cli.export_full_chart_suite(sync_first=False)

        output_dir = Path(result["output_dir"])
        self.assertTrue(output_dir.exists())
        self.assertEqual(result["chart_count"], 6)
        self.assertTrue((output_dir / "equity_curve.png").exists())
        self.assertTrue((output_dir / "filter_impact_visualization.png").exists())
